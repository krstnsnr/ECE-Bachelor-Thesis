...
The chassis and main PCB have already moved to a newer
...
internal state and parameters at runtime. Third, PID gains for steering and
...
design. Chassis, main PCB, and microcontroller have all changed between
...
driven by students and expected to survive repeated crashes. The main PCB
was designed by A. Läßer specifically for this STM32-based generation of
...
board (STM32H533RE microcontroller) mounted on that PCB together with its
...
The new CrazyCar hardware baseline (STM32H533RE, its PCB, the sensor and
...
- Implement drivers and integration for the platform's sensor and actuator
  ...
  measurement unit, an external ADC, a Hall-effect speed sensor, and the
  motor and steering actuation hardware.
...
- Evaluate the resulting platform and report the findings, including
  oversights found in the PCB design during that evaluation.

== Non-Goals <sec:non-goals>

The following are explicitly out of scope for this thesis:

- PCB design for the CrazyCar platform. The main PCB was designed by someone
...
the chassis, the main PCB, the STM32H533RE microcontroller, the
sensor suite, and the motor drivers, together with the PCB oversights found
...
and the usability of the tuning workflow, including the impact of the PCB
oversights on testing. @ch:discussion interprets those results, discusses
