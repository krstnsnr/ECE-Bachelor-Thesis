...
One feature of the STM32H5 series is relevant beyond these specifications. The series includes a
...
The platform uses three ST VL53L1X
...
with a narrow 4 × 4 #gls("roi") to keep its #gls("fov") tight on the track ahead,
while the left and right sensors operate in short mode with a wider 10 × 10
...
alternative NDOF_FMC_OFF mode. In NDOF mode the fusion algorithm
...
register, where 16 #glspl("lsb") correspond to 1 °/s. The linear acceleration, from which the fusion algorithm has already removed the gravity component, is scaled with 100 #gls("lsb") per m/s².
...
field covers the system as a whole, and one field is provided for each of the three
